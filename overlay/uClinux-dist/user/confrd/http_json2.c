    ...
    char *dest, size_t bufsz, size_t *sz)
{
	...
	*path++ = '\0';
	...
	error = http_cmd(sock, "%s /%s HTTP/1.1\r\n", method_str, path);
	if (error)
		return -1;
	error = http_cmd(sock, "Host: %s:%d\r\n", url, port);
	if (error)
		return -1;
	error = http_cmd(sock, "Accept: */*\r\n");
	if (error)
		return -1;
	error = http_cmd(sock, "Content-Length: %d\r\n", (int)data_len);
	if (error)
		return -1;
	if (ctype != NULL) {
		error = http_cmd(sock, "Content-Type: %s\r\n", ctype);
		if (error)
			return -1;
	}
