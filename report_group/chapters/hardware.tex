...
As shown in the bode plot of figure \ref{fig:bode}, the filter will have a strong attenuation on frequencies greater than $50MHz$, which is the maximum input frequency for the ADC (100MSPS sampling rate).

\begin{figure}
 \centering
 \includegraphics[width=0.95\textwidth]{bode}
 \caption{Analog Part Overview.}
 \label{fig:bode}
\end{figure}
...
\hline
\texttt{READ\_META} & \texttt{SEND\_DICMD} & \texttt{words\_stored} \&\& \texttt{req\_type} == \texttt{REQ\_SEND\_DICMD} \\
\hline
\texttt{READ\_META} & \texttt{SETUP\_BITMASK} & \texttt{words\_stored} \&\& \texttt{req\_type} == \texttt{REQ\_SETUP\_BITMASK} \\
\hline
